        ...
        switch ($request->input('action')) {
                        ...
                        foreach($request->line_type_lookup_code as $key =>$line_type_lookup_code){
                            ...
                            if(empty($request->line_id[$key])){
                                ...
                                $data = array(
                                    ...
                                    'rcv_transaction_id'=>$request->receipt_num [$key] ?? '',
                                    'receipt_currency_code'=>$request->customer_currency?? '',
                                    'quantity_invoiced'=>floatval(preg_replace('/[^\d.]/', '', $request->quantity_invoiced[$key])),
                                    'unit_price'=>$request->unit_price [$key],
                                    'account_segment'=>$request->account_segment [$key],
                                    'stat_amount'=>$request->stat_amount [$key],
                                    'amount'=>$amount,
                                ...
                                $data_lines_ap =AccPayTrxLines::find($request->id[$key]);
                                $data_lines_ap->invoice_id=$request->invoice_id;
                                $data_lines_ap->line_id= $request->line_id[$key];
                                $data_lines_ap->item_description=$request->label [$key];
                                $data_lines_ap->account_segment=$request->account_segment [$key];
                                $data_lines_ap->total_rec_tax_amount=$request->total_rec_tax_amount [$key] ?? 0;
                                $data_lines_ap->total_nrec_tax_amount=$request->total_nrec_tax_amount [$key] ?? 0;
                                $data_lines_ap->updated_at=date('Y-m-d H:i:s');

                                if($data_lines_ap->line_type_lookup_code == 0){


                                   // $amount = $request->stat_amount [$key] + $tax;

                                    $data_lines_ap->inventory_item_id=$request->inventory_item_id[$key];

                                    $data_lines_ap->receipt_currency_code=$request->customer_currency ?? '';

                                    $data_lines_ap->quantity_invoiced=floatval(preg_replace('/[^\d.]/', '', $request->quantity_invoiced[$key]));

                                    $data_lines_ap->unit_price=$request->unit_price[$key];
                                    $data_lines_ap->stat_amount=floatval(preg_replace('/[^\d.]/', '', $request->stat_amount[$key]));
                                    $data_lines_ap->original_amount=floatval(preg_replace('/[^\d.]/', '', $request->stat_amount[$key]));
                                    $data_lines_ap->unit_meas_lookup_code=$request->unit_meas_lookup_code[$key];

                                    $data_lines_ap->tax_rate=$request->tax_rate ?? 0;

                                    $data_lines_ap->tax_rate_code=$request->tax_rate_code ?? 'TAX-00';

                                   $data_lines_ap->tax= floatval(preg_replace('/[^\d.]/', '', $request->stat_amount[$key])) * $request->tax_rate;
                    ...
                    if($request->status == 3){
                        ...
                        foreach($request->label as $key =>$label){
                            $data = array(
                                ...
                                'code_combination_id'=>$request->account_segment[$key] ?? '', //
                                'period_name'=>$date,
                                'effective_date'=>$request->duedate ,
                                'status'=>$request->status,
                                'created_by'=>$request->created_by,
                                'entered_dr'=>$request->total_rec_tax_amount[$key] ?? 0, //
                                'entered_cr'=>$request->total_nrec_tax_amount [$key], //
                                ...
                                'tax_code'=>$request->tax_rate_code, //
            ...
            case 'payment':
                try {
                    ...
                    $apUpdate->amount_paid =  ($apUpdate->amount_paid ?? 0)+$amount_paid;
                    $apUpdate->approved_amount =($apUpdate->approved_amount ?? 0) + $amount_paid;
                    $apUpdate->payment_status_flag = $paid_status;
                    $apUpdate->save();
                    ...
                    $payment->invoice_id = $request->invoice_num;
