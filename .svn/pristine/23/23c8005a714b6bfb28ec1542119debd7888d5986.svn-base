...
class SearchController extends Controller
{
    ...
    public function ar_payment(Request $request){
        $payment = ApPayment::where('global_attribute1','Receive')->get();
        $data_arr = array();
        foreach ($payment as $row) {
            if ($row->posted_flag  == 0){
                $status = '<a class="badge bg-secondary text-white">Draft</a>';
    ...
    public function ar_index(Request $request)
    {
        ...
        // Total records
        $totalRecords = \App\ArCustomerTrx::count();
        $totalRecordswithFilter = \App\ArCustomerTrx::select('count(*) as allcount')->where([['id', 'like', '%' . $searchValue . '%']])
        ->count();

        // Get records, also we have included search filter as well
        $records = \App\ArCustomerTrx::orderBy('created_at','desc')
                ->where('bill_template_name','Receivable')
                 ->skip($start)
                 ->take($rowperpage)
                 ->get();
        $data_arr = array();
        foreach ($records as $record) {
            if ($record->status_trx  == 0){
                $status=' <a class="badge bg-secondary text-white">Draft</a>';
            }

        elseif ($record->status_trx == 1){
            $status= '<a class="badge bg-warning text-white">Validate</a>';
        }

        elseif ($record->status_trx == 2){
            $status='<a class="badge bg-success text-white">Account Posted</a>';
        }

        elseif($record->status_trx == 4){
            $status= '<a class="badge bg-primary text-white">'.$record->payment_attributes.'</a>';
        }
            ...
            $data_arr[] = array(
                ...
                "trx_number" => $record->trx_number,
                "trx_date" => $record->trx_date,
                "party_name" => $record->customer->party_name ?? '',
                "address" => $record->party_site->address1 ?? '',
                "attribute1" => $record->attribute1,
                "attribute2" => $record->attribute2,
                "delivery_method_code" => $record->delivery_method_code ?? '',
                "term_id" =>$record->term_id,
                "curr" =>$record->invoice_currency_code,
                "stts" =>  $status ?? '',
                "amount_apl" =>  number_format($record->arapp->amount_applied,2),
                "tax_apl" =>  number_format($record->arapp->tax_applied,2),
                "gl_date" =>  $record->arapp->gl_date ?? '',
            );
        }
