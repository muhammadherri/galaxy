...
class WorkOrderController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        switch ($request->input('action')) {
            case 'save':
                ...
                $planHead->revise = $request->c_roll[0] ?? '';
                ...
                $head->work_order_sub_type='CUSTOM';
                $head->work_order_type='manual';
                ...
                try {
                    ...
                    foreach($request->c_inventory_item_id as $key =>$inventory_item_id){
                        ...
                        $data = array(
                            ...
                            'produced_quantity'=>0,
                            'supply_type'=>$request->c_type_code[$key] ?? 'mtl',
                            'attribute_char1'=>$request->c_roll[$key] ?? 0,
            ...
            case 'auto':
                ...
                foreach($plans as $plan){
                    ...
                    $head->work_order_sub_type='WO';
                    $head->work_order_type='auto';
                    $head->work_order_status_id=1;
                    // dd($head);
                    try {
                        \DB::beginTransaction();
    ...
    public function edit($id){
        ...
        if(count($serial) == 0){
            $serial =   \App\InvOnhandFG::where('id',$wo->planning->revise)->get();
        }
        $pm = OperationUnit::All();
