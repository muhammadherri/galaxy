...
<section class="modern-horizontal-wizard">
    <form action="{{ route("admin.trx-statuses.update", [$status->id]) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
                ...
                <div id="step3" class="content" role="tabpanel" aria-labelledby="step3-trigger">
                    ...
                    <div class="row">
                        <div class="mb-1 col-md-4">
                            ...
                            <input type="text" class="form-control" name="trx_code" placeholder=" Code" required="required" value="{{ $status->trx_code}}">
                        ...
                        <div class="mb-1 col-md-4">
                            <label class="col-sm-4 control-label" for="number">{{ trans('cruds.trx-statuses.fields.trx_name') }}</label>
                             <input type="text" class="form-control" name="trx_name" placeholder="Status Name" value="{{ $status->trx_name}}">
                        </div>
                        <div class="mb-1 col-md-4">
                            <label class="col-sm-4 control-label" for="number">{{ trans('cruds.trx-statuses.fields.trx_function') }}</label>
                              <input type="text" class="form-control" name="trx_function" placeholder="Function" value="{{ $status->trx_function}}">
                        </div>
                    </div>
