...
<div class="card">
    ...
    <div class="card-body">
        <form action="{{ route("admin.gsm.update", [$edit->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
           <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                <label for="name">{{ trans('cruds.planning.fields.item') }} *</label>

                <input type="text" class="form-control search_item_code_r" placeholder="Type here ..."  id="item_code"  readonly value=" {{$edit->item->item_code ??''}}" autocomplete="off" required>
                <span class="help-block search_item_code_empty" style="display: none;" required>No Results Found ...</span>
                <input type="hidden" class="inventory_item_id" id="search_inventory_item_id" name="inventory_item_id" value="{{$edit->inventory_item_id ??''}}" autocomplete="off">
                <input type="hidden" class="form-control search_item_code" value="{{$edit->item_description ??''}}" id="search_item_code" name="item_description" autocomplete="off">
            ...
            <div class="form-group {{ $errors->has('gsm') ? 'has-error' : '' }}">
                ...
                <input type="number" id="gsm" name="gsm" class="form-control" value="{{$edit->gsm ??''}}" readonly required>
                @if($errors->has('value'))
                    <p class="help-block">
                        {{ $errors->first('value') }}
                    </p>
                ...
                <input type="number" id="value" name="value" class="form-control" value="{{$edit->value ??''}}" required>
                @if($errors->has('value'))
                    <p class="help-block">
                        {{ $errors->first('value') }}
                    </p>
            ...
            <div class="form-group">
                ...
                <select id="operation" name="operation" class="form-control select2" readonly>
                    <option value="{{$edit->operation??''}}" selected> {{$edit->operation??''}}</option>
                    @foreach ( $op as  $row)
                    @if ($edit->operation !=$row->short_name)
