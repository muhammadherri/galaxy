...
<section class="modern-horizontal-wizard">
    <form action="{{ route("admin.uom.update",[$uom->id]) }}" method="POST" class="form-horizontal" enctype="multipart/form-data">
    @csrf
    @method('PUT')
            ...
            <div class="bs-stepper-content">
                <div id="step3" class="content" role="tabpanel" aria-labelledby="step3-trigger">
                    <div class="content-header">
                        <h5 class="mb-0">Add Unit Of Measurement Type</h5><br>
                    </div>
                    <div class="row">
                        <div class="mb-1 col-md-4">
                            <label class="form-label">{{ trans('cruds.uom.fields.uom_code') }}</label>
                            <input type="text" name="uom_code" value="{{$uom->uom_code}}" class="form-control" required/>
                        </div>
                        <div class="mb-1 col-md-4">
                            <label class="form-label">{{ trans('cruds.uom.fields.uom_name') }}</label>
                            <input type="text" name="uom_name" value="{{$uom->uom_name}}" class="form-control" required/>
                        </div>
                        <div class="mb-1 col-md-4">
                            <label class="form-label">{{ trans('cruds.uom.fields.description') }}</label>
                            <input type="text" name="description" value="{{$uom->description}}" class="form-control" required/>
                        </div>
                    </div>
