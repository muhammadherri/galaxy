...
class ReceivesController extends Controller
{
    ...
    public function store(StoreRcvRequest $request){
        ...
        $head->conversion_rate=$request->conversion_rate;
		...
		try {
			...
			foreach($request->inventory_item_id as $key =>$inventory_item_id){
                    ...
                    // dd()
                     ...
                     $data->quantity_accepted=$qty_accepted;
                     ...
                     $data->tax_name=\App\Tax::where(['tax_rate'=>$request->tax_id,'type_tax_use'=>'Purchase'])->first()->tax_code;
                     ...
                     $po_data->tax_name=\App\Tax::where('tax_rate',$request->tax_id)->first()->tax_code;
