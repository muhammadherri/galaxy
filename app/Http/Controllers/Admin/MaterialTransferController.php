    ...
    public function store(Request $request)
    {
       ...
       try {
        \DB::beginTransaction();
                        ...
                        $onhandfrom=Onhand::where([['inventory_item_id',$request->inventory_item_id[$key]],['subinventory_code',$request->subinvfrom[$key]],['primary_transaction_quantity','>=',$request->quantity[$key]],['inv_striping_category',$request->category[$key]]])->first();
                        if($onhandfrom && $request->quantity[$key] >0){
                            $update_stock=	$onhandfrom->primary_transaction_quantity-$request->quantity[$key];
                            $onhand=Onhand::where(['inventory_item_id'=>$request->inventory_item_id[$key],'subinventory_code'=>$request->subinvfrom[$key],'inv_striping_category'=>$request->category[$key]])->update(["primary_transaction_quantity"=>$update_stock]);
                        }else {
                            return back()->with('error', 'Stock Zero Or Stock Not Enough '.$request->quantity[$key]);
                        }
                        /* End Of Onhand Transaction From subinventory */

                        // /* Onhand Transaction to subinventory */
							$onhand=Onhand::where(['inventory_item_id'=>$request->inventory_item_id[$key],'subinventory_code'=>$request->subinvto[$key],'inv_striping_category'=>$request->category[$key]])->first();
							if(!$onhand){
								$stock = array(
											...
											'inv_striping_category'=>$request->category[$key],
							...
							Onhand::create($stock);
							 ...
							 $update_stock=	$onhand->primary_transaction_quantity+$request->quantity[$key];
								$onhand=Onhand::where(['inventory_item_id'=>$request->inventory_item_id[$key],'subinventory_code'=>$request->subinvto[$key],'inv_striping_category'=>$request->category[$key]])->update(["primary_transaction_quantity"=>$update_stock]);
							}
