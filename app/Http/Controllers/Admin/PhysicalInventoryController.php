    ...
    public function autoApply(Request $request)
    {
        ...
        try {
            ...
            foreach($request->lines as $key =>$head){
                ...
                $head->name="STJ/".date('Y')."/".date('m')."/".date('d')."(Product Quantity Update - ".$physic->itemmaster->item_code.")";
                $head->created_by=Auth::user()->id;
                $head->last_updated_by=Auth::user()->id;
                $head->je_source="IMPORT";
                $head->status="1";
                $head->je_batch_id=random_int(0, 999999);
                $head->default_effective_date=date('Y-m-d');
                $head->period_name=date('M-y');
                $head->external_reference="STJ/".date('Y')."/".date('m')."/".date('d');
                $head->je_category="Inventory Valuation";
                $head->currency_code='IDR';
                $head->running_total_dr=0;
                $head->running_total_cr=0;
                try {
                    \DB::beginTransaction();
                    $head->save();
                        ...
                        $data = array(
                            ...
                            'ledger_id'=>1,
                            'code_combination_id'=>$physic->itemmaster->category->inventory_account_code,
                            ...
                            'entered_dr'=>0,
                            'entered_cr'=>0,
                            'description'=>$physic->itemmaster->description,
                            'reference_1'=>"-----",
                            'tax_code'=>"TAX-00",
                            'created_at'=>date('Y-m-d H:i:s'),
                            'updated_at'=>date('Y-m-d H:i:s'),
